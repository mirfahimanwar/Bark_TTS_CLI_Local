...
def _generate(
    ...
    Long texts are automatically split into sentence chunks. With rolling=True,
    each chunk is conditioned on the tokens from the previous chunk to keep
    voice and prosody consistent across sentence boundaries (slower).
    With rolling=False (default), each chunk is independently conditioned on
    the static voice profile (faster, standard Bark behaviour).
    """
    import torch
    ...
    if len(chunks) > 1:
        mode = "rolling context" if rolling else "static voice"
        print(f"  (splitting into {len(chunks)} chunks — {mode})")
    ...
    if rolling:
        ...
        for i, chunk in enumerate(chunks):
            ...
            semantic = generate_text_semantic(
                chunk, history_prompt=history_prompt, temp=temp, min_eos_p=0.05,
            )
            coarse = generate_coarse(semantic, history_prompt=history_prompt, temp=temp)
            fine   = generate_fine(coarse, history_prompt=history_prompt, temp=0.5)
        ...
        for i, chunk in enumerate(chunks):
            if len(chunks) > 1:
                print(f"  Chunk {i+1}/{len(chunks)}: \"{chunk[:60]}{'…' if len(chunk)>60 else ''}\"")
            semantic = generate_text_semantic(
                chunk, history_prompt=voice, temp=temp, min_eos_p=0.05,
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Rolling context: condition each chunk on the previous chunk's tokens for "
             "more consistent voice across long texts. Slower than default.",
    )
    # Voice cloning
    parser.add_argument(
        "--clone", metavar="WAV",
